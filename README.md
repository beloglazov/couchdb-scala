...
have a case class called `CouchDoc[T]` that has all the metadata fields
generated by CouchDB and also includes 2 special fields: `doc` for storing an
instance of your domain class `T`, and `kind` for storing a string
representation of the document type that can be used for filtering in views,
lists, and shows (we use `kind` instead of `type` here, as `type` is a reserved
...
it makes a request to the `/awesome-database/_all_docs`
[endpoint](http://docs.couchdb.org/en/1.6.1/api/database/bulk-api.html#get--db-_all_docs). As you can see from the linked documentation page on this endpoint, it has many optional parameters. This builder class provides a fluent interface for constructing queries to this endpoint. For example, to limit the number of documents to the maximum of 10 and returns them in the descending order:
...
`GetDocumentQueryBuilder` making a GET request to the [/{db}/{docid}
endpoint](http://docs.couchdb.org/en/1.6.1/api/document/common.html#get--db-docid). This query builder can accessed through `db.docs.get`.
